...
##
- An critial role of the auditory system is to parse the inputs from the left and right ears into auditory objects - auditory scene analysis. This is an escential role for survival, recognition, and communication.

- Binaural hearing provide us cues that allow us to estimate the relative number and location of sources and objects in the environment.

- These cues also help us to estimate the dimensions and characteristics of rooms as well as hear out speakers in the presence of interfering noise.
...
source of interest if the sounds arrive from the same location (Freyman et al., 1999; Brungart, 2001; Freyman
et al., 2001; Hawley et al., 2004)


>- This task is extreamly challenging for listeners with sensorineural hearing loss (with or without hearing aids) or cochlear implant users (Loizou et al., 2009; Marrone et al., 2008a).


>- Morover, liseners with "normal" hearing and elder listeners experience great difficulty when listening under the presence of bacground noise (Ruggles et al., 2011; Swaminathan et al., 2015; Gallun et al., 2013).
...
We listening to sounds, we rely on three mechanisms for both localize a source and to perform sound source analysis 
...
However, listeners with unilateral hearing loss straggle when the target sound comes from the impaired side, specially in the presence of background noise.
</li> 
...
ITDs are ambiguous for frequencies which wavelengths are smaller than the head 'diameter'.
</li>
...
their auditory filter bandwidths are wider than normal as a result of outer hair cell loss (see Moore 2008). The wider
...
##
<p class="fragment" data-fragment-index="0"> Input to one ear </p>
...
<li class="fragment" data-fragment-index="1">
Interaural time differences conveyed in the fine structure are critical for speech understanding and segregation with in the cocktail party problem (e.g. Swaminathan et al.)
</li>
...
<li class="fragment" data-fragment-index="3">
Objective meaures of binaural processing can be obtained by using stimuli where the temporal fine structure is manipulated so that the perceived location of the sound image changes periodically (e.g. 6.7 Hz) overtime. 
...
By swithcing the "sound image" from left to right at 6.7 Hz, an strong steady-state response is evoked at that particular frequency
</ul>

<ul class="fragment" data-fragment-index="2">
IPM-FRs are larger when the stimuli casuses an strong lateralization percept.
</ul>


## How well do objective IPM-FR and behavioural measuremnts correlate?
...
## Does the IPM-FR differ between healthy and impaired listeners?

<ul class="fragment" data-fragment-index="0"> Vercammen et al. (2018) </ul>

<img src="./my_figures/vercommen_pta.tif" width="60%" class="fragment" data-fragment-index="0">
...
Stimuli were amplitude modulated sinusoidals (500 Hz) using IPDs ranging from 0 (diotic) to 180
...
<img src="./my_figures/vercommen_ipm-fr.png" width="40%">
